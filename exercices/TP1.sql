...
select * from telephones where price > 200 ORDER BY name;
-- *=> Ecrire une requête pour lister les détails(name, manufacturer) de tous les
-- téléphone dont le prix varie entre 150 et 200€
select * from telephones where price >= 150 AND price <= 200 ORDER BY name;
...
select manufacturer, SUM(price), sum(units_sold) from telephones GROUP BY manufacturer ORDER BY manufacturer;
...
-- 1300000€.
select * from telephones where price >1300000;
